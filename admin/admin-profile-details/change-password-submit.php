...
if(isset($_POST['o_pass']) AND isset($_POST['n_pass'])){
	...
	$sql_old_pass_check = "SELECT a_password FROM admin WHERE AID = {$a_id}";
	...
	$run_check_pass = mysqli_query($conn, $sql_old_pass_check);
		...
		$old_pass_db = $get_old_pass['a_password'];
			...
			if(password_verify($old_pass, $old_pass_db)){
				...
				$update_time = $date;
				$sql_change_pass = "UPDATE admin SET a_password = '{$enc_new_pass}', a_updatetime = '{$update_time}' WHERE AID = {$a_id}";
